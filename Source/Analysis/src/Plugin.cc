...
plugin::Configuration Plugin::Configure()
	...
	_set_IDs = DEFAULT_SET_IDS;

	// populate the bin intervals and the normal data
	_binner = std::shared_ptr<Bin_Strategy_Interval> (new Bin_Strategy_Interval());
	_normal_data = std::shared_ptr<std::vector<double> > (new std::vector<double>());
	Bin_Strategy_Interval* ptr = static_cast<Bin_Strategy_Interval*> (_binner.get());
	...
	if ( getline (f, line) )
	{
		...
		for (unsigned int i = 0; i < N; i++)
		{
			...
			try
			{
				_normal_data->push_back(stod(line));
			}
			catch (std::invalid_argument& e)
			{
				cout << e.what() << "\n";
	...
	Val* aid_val, Val* tag_val)
{
	...
	if (aid < 0 || aid > NUMBER_OF_ANALYSIS)
	{
...
void plugin::Analysis_FeatureAnalysis::Plugin::CalculateMetric(Val* Set_ID)
{
	...
	if ((unsigned int) set_ID >= _set_IDs) {_set_IDs = (unsigned int) set_ID + 1;}
	...
	if (return_list.size() > 0) 
	{
		...
		types->append(new TypeDecl(base_type(TYPE_DOUBLE), "value"));
		types->append(new TypeDecl(base_type(TYPE_ENUM), "aid"));
		types->append(new TypeDecl(base_type(TYPE_ENUM), "tag"));
		RecordType* record_type = new RecordType(types);*/
		vl->append(new Val(set_ID, TYPE_ENUM));
		//vl->append(new VectorVal(new VectorType(record_type)));
